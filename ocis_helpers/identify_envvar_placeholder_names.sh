...
# /.make/check-env-var-annotations.sh as part of the ocis CI

RED=$(echo -e "\033[0;31m")
GREEN=$(echo -e "\033[0;32m")
NORM=$(echo -e "\033[0m")

# define the relative path to start searching
# the ocis repo clone must be in parallel to the documentation repo
# this must align with the other helper configurations
SEARCH_PATH='../../ocis'

# exclude from grep
EXCLUDE_PATHS='docs/|_test.go|vendor/|vendor-bin/|tools/|internal/|protogen/web/'
...
# query the code
QS=$(grep -rn --include=\*.go "env:" "${SEARCH_PATH}"| grep -v -E "${EXCLUDE_PATHS}" | grep --color=always -P "${USEREGEX}")
