...
. First, locate the name of the service that you wish to access. It is called `ocis` by default, but you may have named it differently in the deployment configuration:
+
[source,bash]
----
docker compose ps -a --format "table {{.Service}}\t{{.State}}\t{{.ID}}"
