...
The next step in the OIDC Code Flow involves an HTTP POST request to the token endpoint of the _Infinite Scale Identity Server_.
+
--
[source,bash]
----
...
When using the implicit flow, tokens are provided in a URI fragment of the redirect URL. Valid values for the `response_type` request parameter are:
...
[WARNING]
====
If you are using the implicit flow, `nonce` parameter is required in the initial `/authorize` request. `nonce=pL3UkpAQPZ8bTMGYOmxHY/dQABin8yrqipZ7iN0PY18=`
====
