...
* `search(term: string)` - Function that exectues the search, based on a given term. The term is formatted in link:https://owncloud.dev/services/search/#query-language[KQL]. Please note that the returned values needs to be formatted to fit either `SearchResource` or `GenericSearchResultItem` type
...
The following example shows how a search extension that queries a Solr search engine could look like. Note that the extension is wrapped inside a Vue composable so it can easily be reused. All helper types and composables are being provided via the link:https://github.com/owncloud/web/tree/master/packages/web-pkg[web-pkg] and the link:https://github.com/owncloud/web/tree/master/packages/web-client[web-client] packages.
