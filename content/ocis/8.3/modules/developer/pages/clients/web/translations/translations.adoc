...
The following files are managed via `make` commands in all folders containing translations, see section xref:#translation-process[Translation Process] for more details.
...
. The source text such as `$gettext('Administration Settings')`.
. For each language, the minimum translation completion percentage required for processing. +
...
. Languages that will be processed by the make commands are defined in `/gettext.config.cjs`.
. Languages shown in the webUI including their user-facing text are defined in +
...
A nightly sync automatically extracts and synchronizes data, see the link:https://owncloud.dev/services/general-info/add-translations/[Add Translations] documentation for more details, but it can also be triggered manually. Here are the steps for a manual process, run the commands from the repos root:
