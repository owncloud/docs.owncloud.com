...
Our end-to-end test suite is built upon the link:https://github.com/microsoft/playwright[Playwright Framework],
which makes it easy to write tests, debug them and have them run cross-browser with minimal overhead.

=== Preparation

Please make sure you have installed all dependencies and started the server(s) as described in xref:clients/web/development/tooling.adoc#development-setup[tooling].
...
There's a documentation to serve link:https://owncloud.dev/ocis/deployment/ocis_keycloak/[oCIS with Keycloak]. Please follow each step to run *oCIS with Keycloak*.
