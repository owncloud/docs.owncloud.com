...
++jwt ++
a| [subs=-attributes]
Sets how OIDC access tokens should be verified. Possible values are 'none' and 'jwt'. When using 'none', no special validation apart from using it for accessing the IDP's userinfo endpoint will be done. When using 'jwt', it tries to parse the access token as a jwt token and verifies the signature using the keys published on the IDP's 'jwks_uri'.

a|`PROXY_OIDC_ACCESS_TOKEN_VERIFY_AUD` +
...
++ ++
a| [subs=-attributes]
The name of the OIDC claim that holds the groups. When empty (the default), group membership sync is disabled and groups are not created from claim values. Set it (e.g. to 'groups') to enable syncing and creating groups from the claim.
