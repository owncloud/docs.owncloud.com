...
itself an access token, or it is refused with `Token is not an access token`. Either the `typ` or
the `token_use` payload claim counts as that label, and the accepted values, compared
case-insensitively, are `Bearer`, `at+jwt` (RFC 9068 section 2.1), `access` and `access_token` -
any of them in either claim. That is an allowlist rather than a list of refresh markers, so it
