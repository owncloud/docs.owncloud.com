...
for any provider whose shape is known - see the table. Four shapes it does lock out. Three of them
cannot be rescued with `audience`: an introspection response carrying neither `aud` nor
`client_id`, which RFC 7662 permits; an access token labelled with a `typ` or `token_use` value
that is not on the allowlist below; and a JWT access token with no usable `exp` claim, refused with
`Access token has no expiry`. The fourth - a token whose most authoritative client claim names
something other than the `client-id`, even where a lower-precedence one names it - is rescuable,
because setting `audience` turns the client claims off altogether and `aud` alone decides. An access token's `aud` identifies the
