...
* **2.3.5** accepts a token whose `aud` (audience) claim carries the configured `client-id`, or
  ...
  what your provider really puts in `aud`. Only the most authoritative of those three claims that
  the token actually carries is consulted, in that order, so a token whose `azp` names a different
  client is refused even when it also carries a `client_id` naming ownCloud. It checks both JWT
  access tokens and opaque ones verified through a token introspection endpoint, where the
  response's `client_id` counts the same way.

Both halves therefore arrive together here, which is why the upgrade needs no configuration change
for any provider whose shape is known - see the table. The one shape it can lock out is an
introspection response carrying neither `aud` nor `client_id`, which RFC 7662 permits and which no
`audience` value can rescue. An access token's `aud` identifies the
