...
* **2.3.5** accepts a token whose `aud` (audience) claim carries the configured `client-id`, or
  ...
  what your provider really puts in `aud`. `aud` is compared first and a match there ends the
  ...
  carries a `client_id` naming ownCloud. It checks both JWT
...
for any provider whose shape is known - see the table. Three shapes it does lock out, none of them
rescuable with `audience`: an introspection response carrying neither `aud` nor `client_id`, which
RFC 7662 permits; an access token labelled with a `typ` or `token_use` value that is not on the
allowlist below; and a token whose most authoritative client claim names something other than the
`client-id`, even where a lower-precedence one names it. An access token's `aud` identifies the
